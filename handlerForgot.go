...
func sendReset(c *gin.Context) {
	...
	log.Println("page sendReset password for ", body.Email)
	...
	log.Println("find user from db ", body.Email, "err: ", err)
	if err != nil {
	...
	log.Println("page renderForgot url: %s", resetURL)
	mailConf := MailConfig{}
